...
import utry.psd.call.center.test.ComplianceDateCreate;
 ...
 * @author sharkTang
 */
@ServerEndpoint(value = "/complinanceServerServer")
@Component
public class ComplinanceServerWebSocketServer extends UtryWebSocketServer {
	private Logger logger_;

	public ComplinanceServerWebSocketServer() {
	...
	public void onOpen(Session session) {
		try {
			...
			webSocketAction(webSocketMap, session, WebSocketConstants.ADD_STR,
					webSocketBo);
			// 发送初始化数据
			String sendDataString = this.getSendData();
			//System.out.println("sendDataString: " + sendDataString);
			if (null != sendDataString && !"".equals(sendDataString)) {
	...
	private static String getStatusData() {
		List<List<Object>> objs = new ArrayList<List<Object>>();
		List<Object> list2 = ComplianceDateCreate.makeComplianceVoData(null);
		List<Object> list3 = ComplianceDateCreate.makeComplianceCurrentTimeVo();
		objs.add(list2);
		objs.add(list3);
	...
	private static String getSendData() {
		List<List<Object>> objs = new ArrayList<List<Object>>();
		List<Object> list1 = ComplianceDateCreate.makeCompliancePlanVo();
		List<Object> list2 = ComplianceDateCreate.makeComplianceVoData(list1);
		List<Object> list3 = ComplianceDateCreate.makeComplianceCurrentTimeVo();
		objs.add(list1);
		objs.add(list2);
		objs.add(list3);
